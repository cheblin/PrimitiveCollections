...
public interface UByteUIntMap {
	...
	interface Producer {
		...
		long produce_val(@Positive_ONLY int state);
	...
	class R implements Cloneable, Comparable<R>, Producer {
		...
		protected R(int length)                   { values = new UIntList.RW(265 < length ? 256 : length); }
		...
		@Override public long produce_val(int state) { return (0xFFFFFFFFL &  values.array[state >> 8]); }
