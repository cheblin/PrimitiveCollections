...
public interface UByteByteNullMap {
	...
	interface Reader {
		...
		byte read_val(@Positive_ONLY int info);
	...
	class R implements Cloneable, Comparable<R>, Reader {
		...
		protected R(int length)                            { values = new ByteNullList.RW(265 < length ? 256 : length); }
		...
		@Override public byte read_val(@Positive_ONLY int info) { return  values.value(info >> 8); }
	...
	class RW extends R implements Writer {
		...
		@Override public boolean put( Character key, byte value) {
		...
		@Override public boolean put(char key, byte value) {
