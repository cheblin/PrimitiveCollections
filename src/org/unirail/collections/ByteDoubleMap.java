...
public interface ByteDoubleMap {
	...
	interface Consumer {
		boolean put( byte key, double value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public R( int length ) {
			values = new DoubleList.RW( 265 < length ? 256 : length );
		}
		...
		public double  get( int tag ) {return    values.array[keys.rank( (byte) tag )];}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public double value( int tag ) { return  values.array[tag & 0xFFFF]; }
