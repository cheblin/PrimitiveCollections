...
public interface IntShortNullMap {
	...
	interface NonNullKeysIterator {
		...
		static int key(R src, int token) {return token == Positive_Values.VALUE - 1 ? 0 : src.keys[token & Integer.MAX_VALUE];}
		
		static boolean hasValue(R src, int token) {return -1 < token;}
		
		static short value(R src, int token) {return token == Positive_Values.VALUE - 1 ? src.OKeyValue :    src.values.get(token);}
	}
