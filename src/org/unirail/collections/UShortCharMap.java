...
public interface UShortCharMap {
	...
	interface NonNullKeysIterator {
		...
		static char key(R src, int token) {return token == -2 ? 0 :   src.keys.array[token];}
		
		static char value(R src, int token) {return token == -2 ? src.OkeyValue :  (char) src.values.array[token];}
	}
		...
		public boolean equals(R other) {
			...
			for (int i = keys.array.length, c; -1 < --i; )
				if ((key =  keys.array[i]) != 0)
				{
					...
					if (other.value(c) !=  (char) values.array[i]) return false;
				}
