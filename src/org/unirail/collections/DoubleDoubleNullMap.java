...
public interface DoubleDoubleNullMap {
	public interface Consumer {
		boolean put( double key, double value );
		
		boolean put( double key );
	}
	...
	public static class R implements Cloneable, Comparable<R> {
		...
		public boolean contains( double key ) {return -1 < tag( key );}
		
		
		public double get( int tag ) { return tag == Nullable.VALUE ? OKeyValue : values.get( tag ); }
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public double key( int tag ) {return (tag &= Integer.MAX_VALUE) == keys.array.length ? (double) 0 :   keys.array[tag]; }
				
				public double value( int tag ) {return tag == keys.array.length ? OKeyValue :  values.get( values.tag( tag ) ) ; }
			} : producer;
		}
