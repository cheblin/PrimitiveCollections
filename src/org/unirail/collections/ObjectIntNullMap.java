	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<K>() {
				...
				public int value(int tag) {return tag < keys.array.length ? values.get(values.tag(tag)) : NullValue; }
		...
		public int get(@Nullable int tag) { return tag == Nullable.VALUE ? NullValue : values.get(tag); }
