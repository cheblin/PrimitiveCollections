...
public interface ObjectIntNullMap {
	...
	class Entry<K extends Comparable<? super K>> {
		...
		public Entry( K key,  Integer   value ) {
			this.key   = key;
			this.value = value;
		}
	}
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		protected boolean put( K key, int value ) {
		...
		protected void allocate( int size ) {
			...
			values = new IntNullList.RW( size + 1 );
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<>() {
				...
				public int value( int tag ) {return tag == keys.array.length ? NullKeyValue :  values.get( values.tag( tag ) ) ; }
		...
		public int get( int tag ) { return tag == Nullable.VALUE ? NullKeyValue : values.get( tag ); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		public boolean put( K key, int value ) { return super.put( key, value ); }
