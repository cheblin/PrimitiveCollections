...
public interface ShortList {
	...
	class Rsize extends R {
		...
		public void set( int index, short value ) { if (index < size) array[index] = (short) value;}
		
		public void set( int index, int... src ) {
			for (int i = 0, max = Math.min( src.length, size - index ); i < max; i++)
			     array[index + i] = (short) src[i];
		}
	}
	...
	class RW extends R implements Array, Consumer {
		...
		public void addAll( Producer src ) {
			for (int tag = src.tag(), i = size; src.ok( tag ); tag = src.tag( tag )) array[i++] = (short) src.value( tag );
		}
		
		public boolean addAll( Producer src, int index ) {
			for (int tag = src.tag(); src.ok( tag ); tag = src.tag( tag )) array[index++] = (short) src.value( tag );
			return true;
		}
