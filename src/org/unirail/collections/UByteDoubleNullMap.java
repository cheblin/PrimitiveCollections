...
public interface UByteDoubleNullMap {
	...
	interface NonNullKeysIterator {
		...
		static char key( R src,  int token ) {return (char) (token & 0xFF);}
		
		static boolean hasValue( R src, int token ) {return -1 < token;}
		
		static double value( R src, int token ) {return  src.values.get( token >> 8 );}
	}
	...
	class RW extends R {
		...
		public boolean put( char key,  Double    value ) {
			...
			values.set( keys.rank( (byte) key ) - 1, ( Double   ) null );
