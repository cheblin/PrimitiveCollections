...
public interface ObjectUByteNullMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>>, Producer<K> {
		...
		UByteNullList.RW values = new UByteNullList.RW(0);
		...
		public char value(@Positive_ONLY int contains) { return contains == Positive_Values.VALUE ? NullKeyValue : values.value(contains); }
		...
		@Override public char produce_val(@Positive_ONLY int state) {return values.value(state); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		@Override public boolean put(K key,  Character value) {
		...
		@Override public boolean put(K key, char value) {
		...
		protected void allocate(int size) {
			...
			UByteNullList.RW vals = values;
			
			keys.length(-size);
			values = new UByteNullList.RW(-size);
