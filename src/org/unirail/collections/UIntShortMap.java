...
public interface UIntShortMap {
	...
	interface NonNullKeysIterator {
		...
		static long key(R src, int token) {return token == -2 ? 0 :   src.keys[token];}
		
		static short value(R src, int token) {return token == -2 ? src.OkeyValue :  (short) src.values[token];}
	}
	
	
	abstract class R implements Cloneable {
		int[] keys   = Array.ints0     ;
		short[] values = Array.shorts0     ;
		...
		public @Positive_Values int token(long key) {
			...
			for (int key_ = (int) key , k; (k = keys[slot]) != 0; slot = slot + 1 & mask)
