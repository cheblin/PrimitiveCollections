...
public interface ByteFloatMap {
	...
	interface Consumer {
		boolean put( byte key, float value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public float  get( int tag ) {return    values.array[keys.rank( (byte) tag )];}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public float value( int tag ) { return  values.array[tag & 0xFFFF]; }
