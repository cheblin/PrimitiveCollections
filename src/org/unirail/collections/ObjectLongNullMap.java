...
public interface ObjectLongNullMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		protected boolean put( K key, long value ) {
		...
		protected void allocate( int size ) {
			...
			values = new LongNullList.RW( size + 1 );
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<>() {
				...
				public long value( int tag ) {return tag == keys.array.length ? NullKeyValue :  values.get( values.tag( tag ) ) ; }
		...
		public long get( int tag ) { return tag == Nullable.VALUE ? NullKeyValue : values.get( tag ); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		public boolean put( K key, long value ) { return super.put( key, value ); }
