...
public interface ShortIntNullMap {
	interface Writer {
		boolean put(short key, int value);
		
		boolean put(short key,  Integer   value);
	...
	class R implements Cloneable, Comparable<R>, Reader {
		...
		@Override public short read_key(int info) {return  (short) keys.array[info & Integer.MAX_VALUE]; }
	...
	class RW extends R implements Writer {
		...
		void allocate(int size) {
			...
			for (int i = array.length - 1; -1 < --i; )
				if ((key = array[i]) != 0)
					if (values.nulls.value(i)) tmp.put((short) key, values.value(i));
