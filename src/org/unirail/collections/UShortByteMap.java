...
public interface UShortByteMap{
	...
	interface Consumer {
		boolean put( char key, byte value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public byte value( int tag ) {return assigned == 0 || tag == keys.array.length ? OKeyValue : (byte) values.array[tag]; }
			} : producer;
		}
