...
public interface LongList {
	...
	class Rsize extends R {
		...
		public void set( int index, long value ) { if (index < size) array[index] = (long) value;}
		
		public void set( int index, long... src ) {
			for (int i = 0, max = Math.min( src.length, size - index ); i < max; i++)
			     array[index + i] = (long) src[i];
		}
	}
	...
	class RW extends R implements Array, Consumer {
		...
		public void addAll( Producer src ) {
			for (int tag = src.tag(), i = size; src.ok( tag ); tag = src.tag( tag )) array[i++] = (long) src.value( tag );
		}
		
		public boolean addAll( Producer src, int index ) {
			for (int tag = src.tag(); src.ok( tag ); tag = src.tag( tag )) array[index++] = (long) src.value( tag );
			return true;
		}
