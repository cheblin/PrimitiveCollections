...
public interface ObjectShortNullMap {
	interface Iterator{
		...
		static <K extends Comparable<? super K>> short value( R<K> src, @Positive_ONLY int token ) {return src.values.get(token); }
	}
	...
	abstract class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public short value(@Positive_ONLY int token) { return token == Positive_Values.VALUE ? NullKeyValue : values.get(token); }
