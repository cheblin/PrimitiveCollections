...
public interface ObjectShortNullMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>>, Producer<K> {
		...
		ShortNullList.RW values = new ShortNullList.RW(0);
		...
		public short value(@Positive_ONLY int contains) { return contains == Positive_Values.VALUE ? NullKeyValue : values.value(contains); }
		...
		@Override public short produce_val(@Positive_ONLY int state) {return values.value(state); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		@Override public boolean put(K key,  Short     value) {
			if (value != null) put(key, (short) value);
		...
		protected void allocate(int size) {
			...
			keys.length(-size);
			values = new ShortNullList.RW(-size);
			...
			for (int i = k.length - 1; 0 <= --i; )
				if ((kk = k[i]) != null)
				{
					...
					else values.set(slot, ( Short    ) null);
				}
