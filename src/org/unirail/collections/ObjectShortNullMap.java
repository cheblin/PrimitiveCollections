	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<>() {
				...
				public short value( int tag ) {return tag < keys.array.length ? values.get( values.tag( tag ) ) : NullValue; }
		...
		public short get( @Nullable int tag ) { return tag == Nullable.VALUE ? NullValue : values.get( tag ); }
		...
		protected void allocate( int size ) {
			...
			for (int i = k.length - 1; 0 <= --i; )
				if ((kk = k[i]) != null)
				{
					...
					if (tag < 0) values.set( slot,( Short    )null );
