...
public interface ShortObjectMap {
	...
	public interface Consumer<V extends Comparable<? super V>> {
		boolean put( short key, V value );//return false to interrupt
		
		boolean put(  Short     key, V value );
	...
	public static class R<V extends Comparable<? super V>> implements Cloneable, Comparable<R<V>> {
		...
		protected int hashKey( short key ) { return Array.hash( key ); }
		...
		public Producer<V> producer() {
			return producer == null ? producer = new Producer<V>() {
				...
				public short key( int tag ) {return tag < keys.array.length ?(short) keys.array[tag] : (short) 0; }
