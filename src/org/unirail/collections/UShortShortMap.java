...
public interface UShortShortMap {
	...
	interface NonNullKeysIterator {
		...
		static short value(R src, int token) {return token == -2 ? src.OkeyValue :  (short) src.values.array[token];}
	}
		...
		public boolean equals(R other) {
			...
			for (int i = keys.array.length, c; -1 < --i; )
				if ((key =  keys.array[i]) != 0)
				{
					...
					if (other.value(c) !=  (short) values.array[i]) return false;
				}
