...
public interface DoubleShortNullMap {
	...
	interface NonNullKeysIterator {
		...
		static double key(R src, int token) {return token == -2 ? 0 : src.keys[token & Integer.MAX_VALUE];}
		
		static boolean hasValue(R src, int token) {return (token == -2 && src.has0key == Positive_Values.VALUE) || -1 < token;}
		
		static short value(R src, int token) {return token == -2 ? src.OkeyValue :    src.values.get(token);}
	}
