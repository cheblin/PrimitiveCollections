...
public interface ObjectShortMap {
	...
	interface Consumer<K extends Comparable<? super K>> {
		boolean put( K key, short value );//return false to interrupt
	}
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public short get( int tag ) {return tag == Integer.MAX_VALUE ? NullKeyValue :   values.array[tag]; }
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<>() {
				...
				public short value( int tag ) {return assigned == 0 || tag == keys.array.length ? NullKeyValue :  values.array[tag]; }
