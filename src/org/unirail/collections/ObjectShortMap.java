...
public interface ObjectShortMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public short get( int tag ) {return tag == Integer.MAX_VALUE ? NullValue :  (short) values.array[tag]; }
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<K>() {
				...
				public short value( int tag ) {return assigned == 0 || tag == keys.array.length ? NullValue : (short) values.array[tag]; }
