...
public interface ShortShortNullMap {
	...
	interface Iterator {
		...
		static short key( R src, int token ) {return  (short) src.keys.array[token & Integer.MAX_VALUE];}
		...
		static short value( R src, @Positive_ONLY int token ) {return   src.values.get( token );}
	}
	...
	abstract class R implements Cloneable, Comparable<R> {
		...
		public int hashCode() {
			...
			for (int i = keys.array.length; -1 < --i; )
				if ((k = keys.array[i]) != 0)
					hash = Hash.code( Hash.code( hash, (short) k  ), values.hasValue( i ) ? values.get( i ) : 528491 );
