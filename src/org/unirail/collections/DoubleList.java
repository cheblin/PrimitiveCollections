...
public interface DoubleList {
	...
	class R implements Comparable<R> {
		...
		R( int length ) {
			if (0 < length) array = new double[length];
		}
		...
		static void fill( R dst, double... items ) {
			dst.array = new double[dst.size = items.length];
			
			for (int i = 0; i < dst.size; i++)
			     dst.array[i] = (double) items[i];
		}
		...
		public R subList( int fromIndex, int toIndex, R dst ) {
			...
			else if (dst.length() < toIndex - fromIndex) dst.array = new double[toIndex - fromIndex];
	...
	class RW extends Rsize implements Array, Consumer {
		...
		public double[] length( int items ) { return array = items == 0 ? null : new double[items];}
		...
		public void addAll( Producer src ) {
			for (int tag = src.tag(), i = size; src.ok( tag ); tag = src.tag( tag )) array[i++] = (double) src.value( tag );
		}
		
		public boolean addAll( Producer src, int index ) {
			for (int tag = src.tag(); src.ok( tag ); tag = src.tag( tag )) array[index++] = (double) src.value( tag );
			return true;
		}
