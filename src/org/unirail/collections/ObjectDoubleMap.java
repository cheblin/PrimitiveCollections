...
public interface ObjectDoubleMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public double get( int tag ) {return tag == Integer.MAX_VALUE ? NullValue :   values.array[tag]; }
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<K>() {
				...
				public double value( int tag ) {return assigned == 0 || tag == keys.array.length ? NullValue :  values.array[tag]; }
