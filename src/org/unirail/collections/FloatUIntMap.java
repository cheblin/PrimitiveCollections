...
public interface FloatUIntMap {
	...
	class R implements Cloneable, Comparable<R> {
		...
		public @Nullable int tag(  Float     key ) {return key == null ? hasNull ? Nullable.NULL : Nullable.NONE : tag( (float) key );}
		...
		public long get( @Nullable int tag ) {
			...
			return  (0xFFFFFFFFL &  values.array[tag]);
		}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public long value( int tag ) {return assigned == 0 || tag == keys.array.length ? OValue : (0xFFFFFFFFL &  values.array[tag]); }
