...
public interface CharIntNullMap {
	interface Consumer {
		...
		boolean put( Character key,  Integer   value);
	...
	class R implements Cloneable, Comparable<R>, Producer {
		...
		@Override public char produce_key(int info) {return  (char) keys.array[info & Integer.MAX_VALUE]; }
	...
	class RW extends R implements Consumer {
		...
		void allocate(int size) {
			...
			for (int i = array.length - 1; -1 < --i; )
				if ((key = array[i]) != 0)
					if (values.nulls.value(i)) tmp.put((char) key, values.value(i));
