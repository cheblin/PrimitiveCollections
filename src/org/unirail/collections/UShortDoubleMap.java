...
public interface UShortDoubleMap{
	...
	interface Consumer {
		boolean put( char key, double value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public double value( int tag ) {return assigned == 0 || tag == keys.array.length ? OKeyValue :  values.array[tag]; }
			} : producer;
		}
