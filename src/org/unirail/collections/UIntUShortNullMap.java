...
public interface UIntUShortNullMap {
	interface Consumer {
		...
		boolean put( long key,  Character value );
		
		boolean put(  Integer   key,  Character value );
	...
	class R implements Cloneable, Comparable<R> {
		...
		public @Nullable int tag(  Integer   key ) {return key == null ? hasNull : tag( (long) (key + 0) );}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public long key( int tag ) {return (tag &= Integer.MAX_VALUE) < keys.array.length ?  keys.array[tag] : (long) 0; }
