...
public interface ObjectUByteMap {
	...
	abstract class R<K> implements Cloneable, JsonWriter.Source {
		...
		public char value( @Positive_ONLY int token ) { return token == keys.length + 1 ? NullKeyValue :  (char)( 0xFFFF &  values[token]); }
		...
		public void build_V( Array.ISort.Primitives.Index dst ) {
			...
			dst.src = getV == null ? getV = index -> (char)( 0xFFFF &  values[index]) : getV;
