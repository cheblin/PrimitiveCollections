...
public interface ObjectUByteMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public char get( int tag ) {return tag == Integer.MAX_VALUE ? NullValue :  (char)( 0xFFFF &  values.array[tag]); }
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<K>() {
				...
				public char value( int tag ) {return assigned == 0 || tag == keys.array.length ? NullValue : (char)( 0xFFFF &  values.array[tag]); }
