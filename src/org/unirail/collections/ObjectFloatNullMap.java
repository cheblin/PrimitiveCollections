...
public interface ObjectFloatNullMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		protected boolean put( K key, float value ) {
		...
		protected void allocate( int size ) {
			...
			values = new FloatNullList.RW( size + 1 );
		...
		public float get( int tag ) { return tag == Nullable.VALUE ? NullKeyValue : values.get( tag ); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		public boolean put( K key, float value ) { return super.put( key, value ); }
