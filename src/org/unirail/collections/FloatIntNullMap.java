...
public interface FloatIntNullMap {
	interface Consumer {
		boolean put(float key, int value);
		
		boolean put(float key,  Integer   value);
	...
	interface Producer {
		...
		int produce_val(@Positive_ONLY int info);
	...
	class R implements Cloneable, Comparable<R>, Producer {
		...
		public @Positive_Values int info( Float     key) {return key == null ? hasNullKey : info((float) (key + 0));}
		...
		public int value(@Positive_ONLY int info) {
		...
		@Override public int produce_0key_val() {return OkeyValue;}
		
		@Override public float produce_key(int info) {return   keys.array[info & Integer.MAX_VALUE]; }
		...
		@Override public int produce_val(@Positive_ONLY int info) {return   values.value(info); }
