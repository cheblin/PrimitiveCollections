...
public interface IntUByteMap {
	...
	interface NonNullKeysIterator {
		...
		static int key(R src, int token) {return token == Positive_Values.VALUE - 1 ? 0 :   src.keys[token];}
		
		static char value(R src, int token) {return token == Positive_Values.VALUE - 1 ? src.OKeyValue :  (char)( 0xFFFF &  src.values[token]);}
	}
