...
public interface ByteNullList {
	...
	interface Producer {
		...
		byte value(@Positive_ONLY int index);
	...
	class R implements Comparable<R>, Producer {
		...
		protected static void fill(R dst, int... values) {
			for (int value : values) dst.values.add((byte) value);
		...
		public byte value( @Positive_ONLY int index) {return (byte) values.array[nulls.rank(index) - 1]; }
	...
	class RW extends R implements Consumer {
		...
		public void set(int index, Integer... values) {
			for (int i = 0, max = values.length; i < max; i++)
				...
				else set(this, index + i, (byte) (values[i] + 0));
		}
