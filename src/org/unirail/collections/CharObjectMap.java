...
public interface CharObjectMap {
	...
	interface NonNullKeysIterator {
		...
		static <V> char key(R<V> src, int token) {return token == -2 ? 0 : (char) src.keys.array[token];}
	...
	abstract class R<V> implements Cloneable {
		...
		public @Positive_Values int token( Character key) {return key == null ? hasNullKey ? Integer.MAX_VALUE : Positive_Values.NONE : token((char) (key + 0));}
