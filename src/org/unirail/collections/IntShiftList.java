...
public interface IntShiftList {
	...
	class RW extends R implements Array, IDst {
		...
		public void set(int index, long... values) {
			...
			for (int i = 0; i < len; i++)
				array[index + i] = (int) (values[i] - shift);
		}
		...
		public void addAll(ISrc src, int index) {
			...
			for (int i = 0; i < s; i++) array[index + i] = (int) (src.get(i) - shift);
		}
