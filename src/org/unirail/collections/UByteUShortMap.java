...
public interface UByteUShortMap {
	...
	class R implements Cloneable, Comparable<R> {
		...
		public R( int length ) {
			values = new UShortList.RW( 265 < length ? 256 : length );
		}
		...
		public char  get( int tag ) {return tag == Nullable.NULL ? NullValue :    values.array[keys.rank( (byte) tag )];}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public char value( int tag ) { return  values.array[tag & 0xFF]; }
