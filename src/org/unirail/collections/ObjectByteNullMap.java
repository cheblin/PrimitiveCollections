...
public interface ObjectByteNullMap {
	interface Iterator{
		...
		static <K extends Comparable<? super K>> byte value( R<K> src, @Positive_ONLY int token ) {return src.values.get(token); }
	}
	...
	abstract class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public byte value(@Positive_ONLY int token) { return token == Positive_Values.VALUE ? NullKeyValue : values.get(token); }
