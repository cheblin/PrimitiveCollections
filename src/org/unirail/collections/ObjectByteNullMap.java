...
public interface ObjectByteNullMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>>, Producer<K> {
		...
		public byte value(@Positive_ONLY int contains) { return contains == Positive_Values.VALUE ? NullKeyValue : values.value(contains); }
		...
		@Override public byte produce_val(@Positive_ONLY int state) {return values.value(state); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		@Override public boolean put(K key,  Byte      value) {
		...
		protected void allocate(int size) {
			...
			keys.length(-size);
			values = new ByteNullList.RW(-size);
			...
			for (int i = k.length - 1; 0 <= --i; )
				if ((kk = k[i]) != null)
				{
					...
					else values.set(slot, ( Byte     ) null);
				}
