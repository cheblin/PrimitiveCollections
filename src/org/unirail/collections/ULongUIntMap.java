...
public interface ULongUIntMap{
	
	interface Consumer {
		boolean put( long key, long value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public long value( int tag ) {return assigned == 0 || tag == keys.array.length ? OKeyValue : (0xFFFFFFFFL &  values.array[tag]); }
			} : producer;
		}
