...
public interface ObjectIntMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>> {
		...
		public int get( int tag ) {return tag == Integer.MAX_VALUE ? NullValue :   values.array[tag]; }
		...
		public Producer<K> producer() {
			return producer == null ? producer = new Producer<>() {
				...
				public int value( int tag ) {return assigned == 0 || tag == keys.array.length ? NullValue :  values.array[tag]; }
