...
public interface FloatNullList {
	...
	class RW extends R {
		...
		public RW( Float     fill_value, int size) {
			this(size);
			if (fill_value == null) nulls.size = size;
			else
			{
				values.size = size;
				float v = (float) (fill_value + 0);
		...
		public RW(float... values) {
			this(values.length);
			for (float value : values) this.values.add((float) value);
		...
		public void length(int length) {
			...
			if (length < values.values.length && f)
			{
				...
				if (c < values.size())
				{
					float[] tmp = values.values;
					
					values.values = new float[length];
