...
public interface ShortDoubleNullMap {
	interface Consumer {
		boolean put( short key, double value );
	...
	class R implements Cloneable, Comparable<R> {
		...
		public @Nullable int tag(  Short     key ) {return key == null ? hasNull : tag( (short) (key + 0) );}
		
		public @Nullable int tag( short key ) {
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public short key( int tag ) {return (tag &= Integer.MAX_VALUE) < keys.array.length ? (short) keys.array[tag] : (short) 0; }
				
				public boolean hasValue( int tag ) { return -1 < tag; }
				
				public double value( int tag ) {return tag < keys.array.length ? values.get( values.tag( tag ) ) : OValue; }
	...
	class RW extends R implements Consumer {
		...
		void allocate( int size ) {
			...
			for (int i = array.length - 1; -1 < --i; )
				if ((key = array[i]) != 0)
					if (values.nulls.contains( i )) tmp.put((short) key, values.get( i ) );
