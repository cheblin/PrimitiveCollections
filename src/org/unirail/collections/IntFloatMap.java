...
public interface IntFloatMap {
	...
	interface IDst {
		boolean put(int key, float value);
		
		boolean put( Integer   key, float value);
	...
	abstract class R implements Cloneable, Comparable<R>, ISrc {
		...
		public @Positive_Values int info( Integer   key) { return key == null ? hasNullKey ? Positive_Values.VALUE : Positive_Values.NONE : info((int) (key + 0));}
		...
		public float value(@Positive_ONLY int info) {
		...
		@Override public float read_0key_val() {return OkeyValue;}
		
		
		@Override public int read(int info)          { for (; ; ) if (keys.array[++info] != 0) return info; }
		
		@Override public int read_key(int info) {return   keys.array[info]; }
		
		@Override public float read_val(int info) {return   values.array[info]; }
	...
	class RW extends R implements IDst {
		...
		@Override public boolean put( Integer   key, float value) {
		...
		@Override public boolean put(int key, float value) {
