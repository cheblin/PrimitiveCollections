...
public interface ObjectDoubleNullMap {
	...
	class R<K extends Comparable<? super K>> implements Cloneable, Comparable<R<K>>, Producer<K> {
		...
		public double value(@Positive_ONLY int contains) { return contains == Positive_Values.VALUE ? NullKeyValue : values.value(contains); }
		...
		@Override public double produce_val(@Positive_ONLY int state) {return values.value(state); }
	...
	class RW<K extends Comparable<? super K>> extends R<K> implements Consumer<K> {
		...
		@Override public boolean put(K key,  Double    value) {
