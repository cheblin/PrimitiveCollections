...
public interface ByteIntMap {
	...
	interface Producer {
		...
		int produce_val(@Positive_ONLY int state);
	...
	class R implements Cloneable, Comparable<R>, Producer {
		...
		protected R(int length)                   { values = new IntList.RW(265 < length ? 256 : length); }
		...
		@Override public int produce_val(int state) { return  values.array[state >> 8]; }
