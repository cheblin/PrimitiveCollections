...
public interface ByteUIntMap {
	...
	interface Consumer {
		boolean put( byte key, long value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public R( int length ) {
			values = new LongList.RW( 265 < length ? 256 : length );
		}
		...
		public long  get( int tag ) {return  (0xFFFFFFFFL &   values.array[keys.rank( (byte) tag )]);}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public long value( int tag ) { return (0xFFFFFFFFL &  values.array[tag & 0xFFFF]); }
