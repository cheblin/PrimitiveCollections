...
public interface ByteUIntMap {
	...
	class R implements Cloneable, Comparable<R> {
		...
		public R( int length ) {
			values = new UIntList.RW( 265 < length ? 256 : length );
		}
		...
		public long  get( int tag ) {return tag == Nullable.NULL ? NullValue :  (0xFFFFFFFFL &   values.array[keys.rank( (byte) tag )]);}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public long value( int tag ) { return (0xFFFFFFFFL &  values.array[tag & 0xFF]); }
