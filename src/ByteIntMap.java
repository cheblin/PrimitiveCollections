...
public interface ByteIntMap {
	...
	interface Consumer {
		boolean put( byte key, int value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public R( int length ) {
			values = new IntList.RW( 265 < length ? 256 : length );
		}
		...
		public int  get( int tag ) {return    values.array[keys.rank( (byte) tag )];}
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public int value( int tag ) { return  values.array[tag & 0xFFFF]; }
