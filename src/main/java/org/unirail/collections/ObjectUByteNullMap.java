...
public interface ObjectUByteNullMap {
	...
	class RW< K > extends R< K > {
		...
		public RW( Array.EqualHashOf< K > equal_hash_K, int capacity ) {
			...
			values            = new UByteNullList.RW( 0 ); // Initial capacity for values. Will be resized in initialize.
		...
		private int resize( int newSize, boolean forceNewHashCodes ) {
			...
			K[]                    old_keys    = keys;
			int[]                  old_hash    = hash;
			UByteNullList.RW old_values  = values.clone(); // Clone values to copy them over
		...
		private int initialize( int capacity ) {
			...
			values   = new UByteNullList.RW( capacity, values == null ?
			                                                 64 :
		 ...
		 */
		private void copy( K key, int hash, char value, boolean hasValue ) {
			int bucketIndex = bucketIndex( hash );
