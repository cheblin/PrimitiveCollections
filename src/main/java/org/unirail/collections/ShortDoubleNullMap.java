...
public interface ShortDoubleNullMap {
	...
	class RW extends R {
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy() ) {
				...
				for( int nulls_index = 0; 0 < old_count; nulls_index++ ) {
					...
					if( bitsKey == -1 ) {
						...
						for( ; key < max; key++ )
							if( ( bitsVal & 1L << key ) != 0 )
								copy( ( short ) key, ( double ) old_values[ key ], true );
							else
								copy( ( short ) key, ( double ) 0, false );
					}
					else
			...
			for( int i = 0; i < old_lo_Size; i++ )
				if( ( ( i >> 6 == nulls_idx ?
					...
					copy( ( short ) old_keys[ i ], ( double ) old_values[ i ], true );
				else
					copy( ( short ) old_keys[ i ], ( double ) 0, false );
		 ...
		 * @param hasValue {@code true} if `value` represents a non-null value, {@code false} if it represents a conceptual null.
		 */
		private void copy( short key, double value, boolean hasValue ) {
			int bucketIndex = bucketIndex( Array.hash( key ) );
			int index       = _buckets[ bucketIndex ] - 1;
			int dst_index;
