 ...
 */
public interface ShortDoubleNullMap {
	...
	class RW extends R {
		...
		public boolean put(  Short     key,  Double    value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( short ) ( key + 0 ), ( double ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( short key, double value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( double  value ) { return put( value, true ); }
		
		public boolean putNullValue( short key ) { return put( key, ( double ) 0, false ); }
		 ...
		 */
		private boolean put( short key, double value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				if( newSize <= flatStrategyThreshold )//switch to hash map strategy
				{
					...
					for( int token = -1; ( token = next1( token + 1, flat_bits ) ) != -1; )
						if( _values.hasValue( token ) )
							...
							put( ( short ) token, ( double ) 0, false );
					flat_bits = null;
				}
				return length();
			}
				...
				values    = new DoubleNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, DoubleNullList.RW old_values, int old_count ) {
