...
public interface ObjectDoubleNullMap {
	...
	class RW< K > extends R< K > {
		...
		public RW( Array.EqualHashOf< K > equal_hash_K, int capacity ) {
			...
			values            = new DoubleNullList.RW( 0 ); // Initial capacity for values. Will be resized in initialize.
		 ...
		 * @return {@code true} if a new key-value pair was inserted, {@code false} if an existing key's value was updated.
		 */
		public boolean put( K key, double value, boolean hasValue ) {
			if( key == null ) {
		...
		private int resize( int newSize, boolean forceNewHashCodes ) {
			...
			K[]                    old_keys    = keys;
			int[]                  old_hash    = hash;
			DoubleNullList.RW old_values  = values.clone(); // Clone values to copy them over
		...
		private int initialize( int capacity ) {
			...
			values   = new DoubleNullList.RW( capacity, values == null ?
			                                                 64 :
		 ...
		 */
		private void copy( K key, int hash, double value, boolean hasValue ) {
			int bucketIndex = bucketIndex( hash );
