 ...
 */
public interface UIntFloatNullMap {
	...
	class RW extends R {
		...
		public boolean put(  Long      key,  Float     value ) {
			return key == null ?
			       ...
			       put( ( long ) ( key + 0 ), ( float ) ( value + 0 ), true );
		}
		 ...
		 * @return `true` if the map's state changed (key added or value status changed).
		 */
		public boolean putNullValue( long key ) { return put( key, ( float ) 0, false ); }
		 ...
		 */
		public boolean put( long key, float value ) { return put( key, value, true ); }
		 ...
		 */
		private boolean put( long key, float value, boolean hasValue ) {
