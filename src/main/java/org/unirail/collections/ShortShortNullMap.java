 ...
 */
public interface ShortShortNullMap {
	...
	class RW extends R {
		...
		public boolean put(  Short     key,  Short     value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( short ) ( key + 0 ), ( short ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( short key, short value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( short  value ) { return put( value, true ); }
		
		public boolean putNullValue( short key ) { return put( key, ( short ) 0, false ); }
		 ...
		 */
		private boolean put( short key, short value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		 ...
		 */
		private boolean put( short value, boolean hasValue ) {
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					ShortNullList.RW _values = values;
					values = new ShortNullList.RW( newSize );
				...
				values    = new ShortNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, ShortNullList.RW old_values, int old_count ) {
