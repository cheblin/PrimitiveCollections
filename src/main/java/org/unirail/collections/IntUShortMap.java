...
public interface IntUShortMap {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		 ...
		 */
		public int key( long token ) { return ( int )  ( keys[ index( token ) ] ); }
	...
	class RW extends R {
			...
			if( index == -1 )  // Bucket is empty: place new entry in {@code hi Region}
				...
				if( _lo_Size <= index ) { // Entry is in {@code hi Region}
					if( keys[ index ] == ( int ) key ) { // Key matches existing {@code hi Region} entry
						values[ index ] = ( char ) value; // Update value
						_version++;
					...
					for( int next = index, collisions = 0; ; ) {
						if( keys[ next ] == key ) {
							values[ next ] = ( char ) value;// Update value
							_version++;
							return false;// Key was not new
						}
		 ...
		 */
		private void copy( int key, char value ) {
