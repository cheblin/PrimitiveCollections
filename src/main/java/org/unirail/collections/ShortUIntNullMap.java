 ...
 */
public interface ShortUIntNullMap {
	...
	class RW extends R {
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					UIntNullList.RW _values = values;
					values = new UIntNullList.RW( newSize );
				...
				values    = new UIntNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, UIntNullList.RW old_values, int old_count ) {
