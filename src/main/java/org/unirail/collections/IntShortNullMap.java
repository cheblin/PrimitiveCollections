	...
	class RW extends R {
		...
		public boolean put(  Integer   key,  Short     value ) {
			return key == null ?
			       ...
			       put( ( int ) ( key + 0 ), ( short ) ( value + 0 ), true );
		}
		...
		private int resize( int newSize ) {
			...
			for( int i = 0; i < old_lo_Size; i++ )
				if( ( ( i < b << 6 ?
					...
					copy( ( int ) old_keys[ i ], ( short ) old_values[ i ], true );
				else
					copy( ( int ) old_keys[ i ], ( short ) 0, false );
		 ...
		 * @param hasValue {@code true} if `value` represents a non-null value, {@code false} if it represents a conceptual null value.
		 */
		private void copy( int key, short value, boolean hasValue ) {
