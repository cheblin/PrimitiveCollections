...
public interface FloatNullList {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		 ...
		 */
		public boolean containsKey( float value ) { return indexOf( value ) != -1; }
	...
	class RW extends R {
		...
		public RW set( int index, float[] src, int src_index, int len ) {
			...
			for( int i = len; -1 < --i; )
			     set( this, index + i, ( float ) src[ src_index + i ] );
			return this;
		}
		...
		protected void switchToFlatStrategy( int capacity ) {
			...
			values = new float[ Math.max( size(), capacity ) ]; // Allocate flat array with sufficient capacity.
