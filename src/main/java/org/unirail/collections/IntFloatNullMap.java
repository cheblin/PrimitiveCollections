 ...
 */
public interface IntFloatNullMap {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		...
		public boolean containsValue(  Float     value ) {
			if( value != null ) return containsValue( ( float ) ( value + 0 ) );
	...
	class RW extends R {
		...
		public boolean put(  Integer   key,  Float     value ) {
			return key == null ?
			       ...
			       put( ( int ) ( key + 0 ), ( float ) ( value + 0 ), true );
		}
		 ...
		 * @return `true` if the map's state changed (key added or value status changed).
		 */
		public boolean putNullValue( int key ) { return put( key, ( float ) 0, false ); }
		 ...
		 */
		public boolean put( int key, float value ) { return put( key, value, true ); }
		 ...
		 */
		private boolean put( int key, float value, boolean hasValue ) {
		...
		private int resize( int newSize ) {
			...
			for( int i = 0; i < old_lo_Size; i++ )
				if( ( ( i < b << 6 ?
					...
					copy( ( int ) old_keys[ i ], ( float ) old_values[ i ], true );
				else
					copy( ( int ) old_keys[ i ], ( float ) 0, false );
		 ...
		 * @param hasValue `true` if mapping to a non-null value, `false` for conceptual null.
		 */
		private void copy( int key, float value, boolean hasValue ) {
			int bucketIndex = bucketIndex( Array.hash( key ) );
			int index       = _buckets[ bucketIndex ] - 1;
			int dst_index;
