	...
	class RW extends R {
		...
		private int initialize( int capacity ) {
			...
			if( flatStrategyThreshold < capacity ) {
				...
				values         = new char[ FLAT_ARRAY_SIZE ];  // 65536
			...
			if( isFlatStrategy ) {
				if( newSize <= flatStrategyThreshold )//switch to hash map strategy
				{
					...
					for( int token = -1; ( token = next1( token + 1, flat_bits ) ) != -1; )
					     put( ( char ) token, (char) _values[ token ] );
					flat_bits = null;
				}
				return length();
			}
				...
				char[] _values = new char[ FLAT_ARRAY_SIZE ];
		 ...
		 */
		private void copy( short[] old_nexts, char[] old_keys, char[] old_values, int old_count ) {
			int new_count = 0;
