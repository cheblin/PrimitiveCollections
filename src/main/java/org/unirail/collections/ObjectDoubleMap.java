...
public interface ObjectDoubleMap {
	...
	class RW< K > extends R< K > {
		 ...
		 */
		public boolean putNullKeyValue( double value ) {
		...
		public boolean put( K key, double value ) {
			...
			if( index == -1 ) dst_index = keys.length - 1 - _hi_Size++; // Calculate new position in hi region
				...
				if( _lo_Size <= index ) { // Existing entry is in {@code hi Region}
					if( this.hash[ index ] == hash && equal_hash_K.equals( keys[ index ], key ) ) { // Key matches existing {@code hi Region} entry
						values[ index ] = ( double ) value; // Update value
						_version++;
				...
				else  // Existing entry is in {@code lo Region} (collision chain)
				{
					...
					for( int next = index; ; ) {
						if( this.hash[ next ] == hash && equal_hash_K.equals( keys[ next ], key ) ) {
							values[ next ] = ( double ) value; // Update value
							return false;// Key was not new
						}
			...
			values[ dst_index ]     = ( double ) value; // Store value
		...
		private int resize( int newSize, boolean forceNewHashCodes ) {
			...
			K[]           old_keys    = keys;
			int[]         old_hash    = hash;
			double[] old_values  = values.clone(); // Clone values to copy them over
		...
		private int initialize( int capacity ) {
			...
			values   = new double[ capacity ]; // Initialize values array
			_lo_Size = 0;
			_hi_Size = 0;
			return capacity;
		}
		 ...
		 */
		private void copy( K key, int hash, double value ) {
			int bucketIndex = bucketIndex( hash );
			...
			values[ dst_index ]     = (double)value; // Store the value
