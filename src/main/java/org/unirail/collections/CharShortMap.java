...
public interface CharShortMap {
	...
	class RW extends R {
			...
			if( isFlatStrategy ) {
				if( newSize <= flatStrategyThreshold )//switch to hash map strategy
				{
					...
					for( int token = -1; ( token = next1( token + 1, flat_bits ) ) != -1; )
					     put( ( char ) token, (short) _values[ token ] );
					flat_bits = null;
				}
				return length();
			}
				...
				short[] _values = new short[ FLAT_ARRAY_SIZE ];
		 ...
		 */
		private void copy( short[] old_nexts, char[] old_keys, short[] old_values, int old_count ) {
			int new_count = 0;
