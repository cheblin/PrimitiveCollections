	...
	class RW extends R {
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					CharNullList.RW _values = values;
					values = new CharNullList.RW( newSize );
				...
				values    = new CharNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, CharNullList.RW old_values, int old_count ) {
