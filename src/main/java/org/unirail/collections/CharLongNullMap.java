 ...
 */
public interface CharLongNullMap {
	...
	class RW extends R {
		...
		public boolean put(  Character key,  Long      value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( char ) ( key + 0 ), ( long ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( char key, long value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( long  value ) { return put( value, true ); }
		
		public boolean putNullValue( char key ) { return put( key, ( long ) 0, false ); }
		 ...
		 */
		private boolean put( char key, long value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					LongNullList.RW _values = values;
					values = new LongNullList.RW( newSize );
				...
				values    = new LongNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, char[] old_keys, LongNullList.RW old_values, int old_count ) {
