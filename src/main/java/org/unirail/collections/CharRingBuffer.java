...
public class CharRingBuffer {
	 ...
	 */
	private static final AtomicLongFieldUpdater< CharRingBuffer > GET = AtomicLongFieldUpdater.newUpdater( CharRingBuffer.class, "get" );
	
	/**
	 * Atomic updater for thread-safe modifications to the put pointer.
	 */
	private static final AtomicLongFieldUpdater< CharRingBuffer > PUT = AtomicLongFieldUpdater.newUpdater( CharRingBuffer.class, "put" );
	...
	public boolean put_multithreaded( char value ) {
		...
		buffer[ ( int ) currentPut & mask ] = ( char ) value;
		return true;
	}
