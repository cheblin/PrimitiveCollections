...
public interface ShortUIntMap {
	...
	class RW extends R {
			...
			if( isFlatStrategy ) {
				if( newSize <= flatStrategyThreshold )//switch to hash map strategy
				{
					...
					for( int token = -1; ( token = next1( token + 1, flat_bits ) ) != -1; )
					     put( ( short ) token, (0xFFFFFFFFL &  _values[ token ]) );
					flat_bits = null;
				}
				return length();
			}
				...
				int[] _values = new int[ FLAT_ARRAY_SIZE ];
		 ...
		 */
		private void copy( short[] old_nexts, short[] old_keys, int[] old_values, int old_count ) {
			int new_count = 0;
