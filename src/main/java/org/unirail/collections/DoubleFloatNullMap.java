	...
	class RW extends R {
		...
		public boolean put(  Double    key,  Float     value ) {
			return key == null ?
			       ...
			       put( ( double ) ( key + 0 ), ( float ) ( value + 0 ), true );
		}
		 ...
		 */
		public boolean putNullValue( double key ) { return put( key, ( float ) 0, false ); }
		...
		private int resize( int newSize ) {
			...
			for( int i = 0; i < old_lo_Size; i++ )
				if( ( ( i < b << 6 ?
					...
					copy( ( double ) old_keys[ i ], ( float ) old_values[ i ], true );
				else
					copy( ( double ) old_keys[ i ], ( float ) 0, false );
		 ...
		 * @param hasValue {@code true} if `value` represents a non-null value, {@code false} if it represents a conceptual null value.
		 */
		private void copy( double key, float value, boolean hasValue ) {
