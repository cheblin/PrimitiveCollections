...
public interface IntIntNullMap {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		...
		public boolean containsValue(  Integer   value ) {
			if( value != null ) return containsValue( ( int ) ( value + 0 ) );
	...
	class RW extends R {
		...
		public boolean put(  Integer   key,  Integer   value ) {
			return key == null ?
			       ...
			       put( ( int ) ( key + 0 ), ( int ) ( value + 0 ), true );
		}
		 ...
		 * @return `true` if the map's state changed.
		 */
		public boolean put( int key,  Integer   value ) {
			return value == null ?
			       putNullValue( ( int ) ( key + 0 ) ) :
			       put( ( int ) ( key + 0 ), ( int ) ( value + 0 ), true );
		}
		 ...
		 * @return `true` if the map's state changed (key added or value status changed).
		 */
		public boolean putNullValue( int key ) { return put( key, ( int ) 0, false ); }
		 ...
		 */
		private boolean put( int key, int value, boolean hasValue ) {
		...
		private int resize( int newSize ) {
			...
			for( int i = 0; i < old_lo_Size; i++ )
				if( ( ( i < b << 6 ?
					...
					copy( ( int ) old_keys[ i ], ( int ) old_values[ i ], true );
				else
					copy( ( int ) old_keys[ i ], ( int ) 0, false );
		 ...
		 * @param hasValue `true` if mapping to a non-null value, `false` for conceptual null.
		 */
		private void copy( int key, int value, boolean hasValue ) {
			int bucketIndex = bucketIndex( Array.hash( key ) );
			int index       = _buckets[ bucketIndex ] - 1;
			int dst_index;
