...
public interface ObjectShortNullMap {
	...
	abstract class R< K > implements JsonWriter.Source, Cloneable {
		...
		* @return A valid token if the key is found, otherwise {@link #INVALID_TOKEN}.
		...
		public short value( long token ) {
			return (short) ( index( token ) == NULL_KEY_INDEX ?
			                    nullKeyValue :
	...
	class RW< K > extends R< K > {
		...
		public RW( Array.EqualHashOf< K > equal_hash_K, int capacity ) {
			...
			values            = new ShortNullList.RW( 0 ); // Initial capacity for values. Will be resized in initialize.
		 ...
		 * @return {@code true} if a new key-value pair was inserted, {@code false} if an existing key's value was updated.
		 */
		public boolean put( K key, short value, boolean hasValue ) {
			if( key == null ) {
		...
		private int resize( int newSize, boolean forceNewHashCodes ) {
			...
			K[]                    old_keys    = keys;
			int[]                  old_hash    = hash;
			ShortNullList.RW old_values  = values.clone(); // Clone values to copy them over
		...
		private int initialize( int capacity ) {
			...
			values   = new ShortNullList.RW( capacity, values == null ?
			                                                 64 :
		 ...
		 */
		private void copy( K key, int hash, short value, boolean hasValue ) {
			int bucketIndex = bucketIndex( hash );
