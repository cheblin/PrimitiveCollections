...
public interface ShortByteNullMap {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		 ...
		 */
		protected short[]         keys      = Array.EqualHashOf.shorts     .O; // Initialized to empty array.
		/**
		 ...
		 * @return {@code true} if the value is found, {@code false} otherwise.
		 */
		public boolean containsValue( byte value ) { return hasNullKey && nullKeyHasValue && nullKeyValue == value || values.indexOf( value ) != -1; }
	...
	class RW extends R {
		...
		public boolean put(  Short     key,  Byte      value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( short ) ( key + 0 ), ( byte ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( short key, byte value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( byte  value ) { return put( value, true ); }
		
		public boolean putNullValue( short key ) { return put( key, ( byte ) 0, false ); }
		 ...
		 */
		private boolean put( short key, byte value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		 ...
		 */
		private boolean put( byte value, boolean hasValue ) {
			boolean b = !hasNullKey;
			
			hasNullKey = true;
			if( nullKeyHasValue = hasValue ) nullKeyValue = ( byte ) value;
			_version++;
			return b;
		}
		...
		public void trim( int capacity ) {
			...
			short[]         old_keys   = keys;
			ByteNullList.RW old_values = values;
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					ByteNullList.RW _values = values;
					values = new ByteNullList.RW( newSize );
				...
				values    = new ByteNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, ByteNullList.RW old_values, int old_count ) {
