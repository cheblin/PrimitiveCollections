...
public interface ShortUByteNullMap {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		 ...
		 */
		protected UByteNullList.RW values;
		/**
	...
	class RW extends R {
		...
		public boolean put(  Short     key,  Character value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( short ) ( key + 0 ), ( char ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( short key, char value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( char  value ) { return put( value, true ); }
		
		public boolean putNullValue( short key ) { return put( key, ( char ) 0, false ); }
		 ...
		 */
		private boolean put( short key, char value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					UByteNullList.RW _values = values;
					values = new UByteNullList.RW( newSize );
				...
				values    = new UByteNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, UByteNullList.RW old_values, int old_count ) {
