...
public interface ObjectULongNullMap {
	...
	class RW< K > extends R< K > {
		...
		public RW( Array.EqualHashOf< K > equal_hash_K, int capacity ) {
			...
			values            = new ULongNullList.RW( 0 ); // Initial capacity for values. Will be resized in initialize.
		...
		private int resize( int newSize, boolean forceNewHashCodes ) {
			...
			K[]                    old_keys    = keys;
			int[]                  old_hash    = hash;
			ULongNullList.RW old_values  = values.clone(); // Clone values to copy them over
		...
		private int initialize( int capacity ) {
			...
			values   = new ULongNullList.RW( capacity, values == null ?
			                                                 64 :
