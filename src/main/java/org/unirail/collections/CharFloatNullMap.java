	...
	class RW extends R {
		...
		public boolean put(  Character key,  Float     value ) {
			return key == null ?
			       ...
			       put( ( char ) ( key + 0 ), ( float ) ( value + 0 ), true );
		}
		 ...
		 */
		public boolean putNullValue( char key ) { return put( key, ( float ) 0, false ); }
		 ...
		 * @return {@code true} if the map was structurally modified (key was added), {@code false} otherwise.
		 */
		public boolean put( char key, float value ) { return put( key, value, true ); }
		 ...
		 */
		private boolean put( char key, float value, boolean hasValue ) {
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy() ) {
				...
				for( int nulls_index = 0; 0 < old_count; nulls_index++ ) {
					...
					if( bitsKey == -1 ) {
						...
						for( ; key < max; key++ )
							if( ( bitsVal & 1L << key ) != 0 )
								copy( ( char ) key, ( float ) old_values[ key ], true );
							else
								copy( ( char ) key, ( float ) 0, false );
					}
					else
			...
			for( int i = 0; i < old_lo_Size; i++ )
				if( ( ( i >> 6 == nulls_idx ?
					...
					copy( ( char ) old_keys[ i ], ( float ) old_values[ i ], true );
				else
					copy( ( char ) old_keys[ i ], ( float ) 0, false );
		 ...
		 * @param hasValue {@code true} if `value` represents a non-null value, {@code false} if it represents a conceptual null.
		 */
		private void copy( char key, float value, boolean hasValue ) {
