...
public interface CharByteNullMap {
	...
	class RW extends R {
		...
		public boolean put(  Character key,  Byte      value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( char ) ( key + 0 ), ( byte ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( char key, byte value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( byte  value ) { return put( value, true ); }
		
		public boolean putNullValue( char key ) { return put( key, ( byte ) 0, false ); }
		 ...
		 */
		private boolean put( char key, byte value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		 ...
		 */
		private void copy( short[] old_next, char[] old_keys, ByteNullList.RW old_values, int old_count ) {
