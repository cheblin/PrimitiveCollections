...
public interface ShortFloatNullMap {
	...
	class RW extends R {
		...
		public boolean put(  Short     key,  Float     value ) {
			return key == null ?
					...
					value == null ?
							...
							put( ( short ) ( key + 0 ), ( float ) ( value + 0 ), true );
		}
		 ...
		 * @return {@code true} if the map state was changed (insert or update).
		 */
		public boolean put( short key, float value ) { return put( key, value, true ); }
		
		
		public boolean putNullKey( float  value ) { return put( value, true ); }
		
		public boolean putNullValue( short key ) { return put( key, ( float ) 0, false ); }
		 ...
		 */
		private boolean put( short key, float value, boolean hasValue ) {
			if( isFlatStrategy ) {
				if( !exists( ( char ) key ) ) {
					exists1( ( char ) key );
					_count++;
		...
		private int resize( int newSize ) {
			...
			if( isFlatStrategy ) {
				...
				{
					FloatNullList.RW _values = values;
					values = new FloatNullList.RW( newSize );
				...
				values    = new FloatNullList.RW( Math.max( _count * 3 / 2, FLAT_ARRAY_SIZE ) );//_count - optimistic
		 ...
		 */
		private void copy( short[] old_next, short[] old_keys, FloatNullList.RW old_values, int old_count ) {
