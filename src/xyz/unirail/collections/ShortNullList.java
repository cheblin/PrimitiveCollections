...
public interface ShortNullList {
	...
	class RW extends R implements Interface {
		...
		public RW(  Short     default_value, int size ) {
			...
			values = new ShortList.RW( default_value_is_null ? (short) 0 : default_value. shortValue     (), size );
		}
		
		public RW( short default_value, long size ) {
			super( false );
			
			nulls  = new BitList.RW( false, (int) size );
			values = new ShortList.RW( default_value, (int) size );
		}
		
		public RW(  Short    ... values ) { this( null, values ); }
		
		public RW(  Short     default_value,  Short    ... values ) {
			super( default_value == null );
			int length = values == null ? 0 : values.length;
			
			nulls       = new BitList.RW( length );
			this.values = new ShortList.RW( length );
			if( length == 0 ) return;
			
			for(  Short     value : values ) add( value );
		}
		
		
		public RW( short... values ) { this( (short)0, values ); }
		public RW( short default_value, short... values ) {
			super( false );
			if( values == null )
			{
				nulls       = new BitList.RW( 0 );
				this.values = new ShortList.RW( 0 );
				return;
			}
			
			nulls       = new BitList.RW( true, values.length );
			this.values = new ShortList.RW( default_value, values );
		}
		...
		public void add( int index,  Short     value ) {
			...
			else add( index, value. shortValue     () );
		}
