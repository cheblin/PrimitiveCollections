...
public interface ObjectUIntMap {
	...
	interface NonNullKeysIterator {
		...
		static < K > long value( R< K > src, int token ) { return   src.values[ token ]; }
	}
	...
	abstract class R< K > implements Cloneable, JsonWriter.Source {
		...
		public long value( @Positive_ONLY int token ) { return token == keys.length + 1 ? NullKeyValue :  (0xFFFFFFFFL &  values[ token ]); }
		...
		public void build_V( Array.ISort.Primitives.Index dst ) {
			...
			dst.src = getV == null ? getV = index -> (0xFFFFFFFFL &  values[ index ]) : getV;
