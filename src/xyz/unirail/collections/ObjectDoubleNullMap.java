...
public interface ObjectDoubleNullMap {
	interface NonNullKeysIterator {
		...
		static < K > double value( R< K > src, int token ) { return src.values.get( token ); }
	}
	...
	abstract class R< K > implements Cloneable, JsonWriter.Source {
		...
		public double value( @Positive_ONLY int token ) { return token == keys.length ? NullKeyValue : values.get( token ); }
		...
		public void build_V( Array.ISort.Primitives.Index dst ) {
			...
			dst.src = getV == null ? getV = (Array.ISort.Primitives.doubles) index  -> values.hasValue( index ) ?  value( index ) : 0 : getV;
	...
	class RW< K > extends R< K > implements Interface< K > {
		...
		public boolean put( K key,  Double    value ) {
			if( value != null ) put( key, ( double ) value );
		...
		protected void allocate( int size ) {
			...
			for( int i = k.length; -1 < --i; )
				if( ( key = k[ i ] ) != null ) {
					...
					else values.set1( slot, (  Double    ) null );
				}
		}
