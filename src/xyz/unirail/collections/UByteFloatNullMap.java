...
public interface UByteFloatNullMap {
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		...
		public float value( @Positive_ONLY int token ) { return token == 256 ? NullKeyValue :    values.get( (byte) token ); }
	...
	class RW extends R implements Interface {
		...
		public boolean put( char key,  Float     value ) {
			...
			{
				values.add1( keys.rank( (byte) key ) - 1, ( Float    ) null );
				return true;
			}
			values.set1( keys.rank( (byte) key ) - 1, ( Float    ) null );
