...
public interface ObjectDoubleMap {
	...
	abstract class R< K > implements Cloneable, JsonWriter.Source {
		...
		public double value( @Positive_ONLY int token ) { return token == keys.length + 1 ? NullKeyValue :   values[ token ]; }
		...
		public void build_V( Array.ISort.Primitives.Index dst ) {
			...
			dst.src = getV == null ? getV = (Array.ISort.Primitives.doubles) index  ->  values[ index ] : getV;
