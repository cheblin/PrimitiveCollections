...
public interface UIntIntNullMap {
	...
	interface NonNullKeysIterator {
		...
		static int value( R src, int token ) { return token == src.keys.length ? src.OKeyValue :    src.values.get( token ); }
	}
	...
	abstract class R implements Cloneable, JsonWriter.Source {
		...
		public int value( @Positive_ONLY int token ) { return token == keys.length + 1 ? NullKeyValue : token == keys.length ? OKeyValue : values.get( token ); }
	...
	interface Interface {
		...
		boolean put( long key,  Integer   value );
