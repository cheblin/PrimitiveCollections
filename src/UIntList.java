...
public interface UIntList {
	...
	class R implements Array,  Comparable<R> {
		...
		public boolean contains( long value ) {
			...
			for (int i = size - 1; -1 < i; i--) if (array[i] == (int)value) return true;
			return false;
		}
	...
	class RW extends R implements Consumer {
		...
		public void addAll( Producer src, int count ) {
			...
			for (int tag = src.tag(), i = size; tag != -1; tag = src.tag( tag )) array[i++] = (int) src.value( tag );
		}
		
		public boolean addAll( Producer src, int index, int count ) {
			
			resize( index, count, false );
			for (int tag = src.tag(); tag != -1; tag = src.tag( tag )) array[index++] = (int) src.value( tag );
			return true;
		}
		...
		public boolean retainAll( Consumer chk ) {
			...
			for (int i = 0, max = size; i < max; i++)
				if (!chk.add( (0xFFFFFFFFL &  array[i]) ))
				{
