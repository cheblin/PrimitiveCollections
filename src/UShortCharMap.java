...
public interface UShortCharMap{
	...
	interface Consumer {
		boolean put( char key, char value );//return false to interrupt
	}
	...
	class R implements Cloneable, Comparable<R> {
		...
		public Producer producer() {
			return producer == null ? producer = new Producer() {
				...
				public char value( int tag ) {return assigned == 0 || tag == keys.array.length ? OKeyValue : (char) values.array[tag]; }
			} : producer;
		}
